...
def dashboard():
    ...
    # get all the parties to show (with user)
...
# ------ a user and their 'created' parties
@app.route("/users/<int:id>/parties")
def user_parties_page(id):
    print(" ID ???????  >>>", id)
    this_user = User.get_by_id_with_parties(id)
    return render_template("user_parties.html", 
                           this_user = this_user)